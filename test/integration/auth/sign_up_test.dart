...
import 'package:mubin_orders/core/di/injection_container.dart' as di;
import 'package:mubin_orders/core/error/api_error_model.dart';
import 'package:mubin_orders/core/network/api_result.dart';
import 'package:mubin_orders/features/auth/sign_up/data/models/signup_req_model.dart';
import 'package:mubin_orders/features/auth/sign_up/data/models/sign_up_res_model.dart';
...
void main() {
  ...
  testWidgets('Successful sign up flow', (tester) async {
    ...
    verify(() => mockSignUpRepo.signUp(
      request: any(
        named: 'request',
        that: isA<SignUpReqModel>().having(
          (r) => r.name,
          'name',
          'Test User',
        ).having(
  ...
  testWidgets('Sign up with invalid data shows error', (tester) async {
    ...
    await tester.enterText(find.byKey(const Key('confirm_password_field')), 'mismatch');
    ...
    await tester.enterText(find.byKey(const Key('confirm_password_field')), 'ValidPass123!');
