...
class SallaOrderCardWidget extends StatelessWidget {
  final List<SallaOrderItemUnitEntity> units;
  final int orderDocsCount;
  final StatEntity stat;
  const SallaOrderCardWidget({
    super.key,
    required this.units,
    required this.orderDocsCount,
    required this.stat,
  });
                ...
                return Column(
                  children: [
                    ListView.builder(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      itemCount: units[index].docs.length,
                      itemBuilder: (context, index) {
                        return SallaOrderItemUnitCardWidget(
                          executionNumber: units[index].executionNumber,
                          doc: units[index].docs[index],
                        );
                      },
                    ),
                  ],
