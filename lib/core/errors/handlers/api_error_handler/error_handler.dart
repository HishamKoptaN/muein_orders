...
class ErrorHandler {
  static ErrorInfo handle({required dynamic error}) {
    ...
    if (error is DioException) {
      ...
      return ErrorInfo(message: _getFirebaseAuthErrorMessage(error.code));
    } else {
  ...
  static ErrorInfo _handleDioException({required DioException error}) {
    ...
    if (error.response != null && error.response!.data != null) {
      try {
        ...
        return const ErrorInfo(message: 'فشل في معالجة بيانات السيرفر');
      }
    }
    String message;
    switch (error.type) {
